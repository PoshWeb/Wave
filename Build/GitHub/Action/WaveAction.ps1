<#
.Synopsis
    GitHub Action for Wave
.Description
    GitHub Action for Wave.  This will:

    * Import Wave
    * If `-Run` is provided, run that script
    * Otherwise, unless `-SkipScriptFile` is passed, run all *.wave.ps1 files beneath the workflow directory
      * If any `-ActionScript` was provided, run scripts from the action path that match a wildcard pattern.
#>

param(
# A PowerShell Script that uses Wave.  
# Any files outputted from the script will be added to the repository.
[string]
$Run,

# The Script File Pattern.
# Scripts in the repository that match this pattern will be run.
[string]
$ScriptFilePattern = '\.wave?\.ps1$',

# If set, will not process any files that match the script file pattern.
[switch]
$SkipScriptFile,
...
function InvokeActionModule {
    ...
    $scriptFiles = @(@(
        Get-ChildItem -Recurse -Path $env:GITHUB_WORKSPACE |
            ...
            Where-Object Name -Match $ScriptFilePattern
        if ($ActionScript) {
    ...
    "Found $($scriptFiles.Length) script files matching '$($ScriptFilePattern)'" | Out-Host
    $scriptFiles | Out-Host
